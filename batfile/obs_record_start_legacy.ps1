...
function Start-Once([string]$TargetUri, [string]$TargetPassword) {
    ...
    try {
        ...
        $start = Send-Request $webSocket "StartRecord"
        if (-not $start.d.requestStatus.result) {
            return 1
        }
...
$result = Start-Once -TargetUri $Uri -TargetPassword $Password
if ($result -ne 0) {
