...
未検証の生引数はマーカーへ書かないでください。有効値は正規化して保存し、不正値は `UNKNOWN` とValidityフラグで表してください。
...
SessionId=<GUID>
ArgsValid=0|1
CaseNoCanonical=<1以上の正規化数値またはUNKNOWN>
TagNormalized=<大文字TagまたはUNKNOWN>
...
新方式で両マーカーが存在しても `SessionId` が一致しない場合は、セッション不一致として通常Case名を使用せず、`CaseUnknown_UNKNOWN` 配下へ保存してください。両マーカーの時刻も信頼せず、両系統を後述の「マーカーなし／不正時」と同じ最新1件フォールバックで処理し、最終終了コードを1にしてください。
...
- 引数不正時は `CaseUnknown_UNKNOWN` 親フォルダをSTART側で作らない。フォールバック保存先はSTOP側で準備する。
...
CaseNoまたはTagが不正、STARTとSTOPのCaseNo／Tagが不一致、またはSTARTマーカー内の引数が不正な場合：
...
マーカーが存在しない、または読めない場合だけは例外です。STOP側引数が有効なら、そのSTOP側引数を保存先名とファイル名に使用し、各保存元の最新1件へフォールバックしてください。マーカー欠落自体はエラーなので、最終終了コードを1にしてください。
...
```text
親フォルダ = Case{最低3桁CaseNo}_{大文字Tag}
子フォルダ = Case{最低3桁CaseNo}_{大文字Tag}#{Repeat}_{DT}
ファイル接頭辞 = Case{最低3桁CaseNo}_{大文字Tag}#{Repeat}
```
...
CaseNo、Tag、Repeatのいずれかが不正、STARTとSTOPのCaseNo／Tagが不一致、STARTマーカー内の引数が不正、または新方式のSessionIdが不一致な場合、親フォルダは次です。
...
└─ CaseUnknown_UNKNOWN
   └─ CaseUnknown_UNKNOWN#2_20260717_143025
      ...
      ├─ CaseUnknown_UNKNOWN#2_<元ファイル名>.png
      ├─ CaseUnknown_UNKNOWN#2_<元ファイル名>.log
      └─ CaseUnknown_UNKNOWN#2_<元ファイル名>.asc
```
...
└─ CaseUnknown_UNKNOWN
   └─ CaseUnknown_UNKNOWN#Unknown_20260717_143025
      ...
      ├─ CaseUnknown_UNKNOWN#Unknown_<元ファイル名>.png
      ├─ CaseUnknown_UNKNOWN#Unknown_<元ファイル名>.log
      └─ CaseUnknown_UNKNOWN#Unknown_<元ファイル名>.asc
```

開始・停止不一致時は、停止操作をすべて実行し、開始マーカーの時刻で今回分を検索しますが、STOP側のCase名では保存しません。`CaseUnknown_UNKNOWN` 配下へ保存し、最終終了コードを1にしてください。

マーカーが存在しない、または読めない場合だけは、STOP側の有効なCaseNo、Tag、Repeatを命名に使用します。対象ファイルは各保存元の最新1件へフォールバックし、最終終了コードを1にしてください。
...
- フォールバック親 `CaseUnknown_UNKNOWN` でも同じ規則を適用する。
...
新方式で片方のマーカーだけが正常な場合は、正常なマーカーを対応ファイル種別に使用し、異常な側だけ最新1件フォールバックへ進めてください。両マーカーのSessionId不一致時は両方を不正扱いにし、命名も`CaseUnknown_UNKNOWN`へ切り替えてください。
...
不一致時も開始マーカーの時刻はファイル選択に使用します。ただし、正常Case名では保存せず、従来方式は日時だけ、新方式は `CaseUnknown_UNKNOWN` へ保存します。
...
```text
CaseUnknown_UNKNOWN#2
```

または：

```text
CaseUnknown_UNKNOWN#Unknown
```
...
- 新方式の引数不正または開始停止不一致は `CaseUnknown_UNKNOWN` 配下。
